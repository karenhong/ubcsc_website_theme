...
<script>
  ...
  function getTableRow(item)
  {
    ...
    return render(
      '<tr ' +
	  (item.isPendingWizardOperation ? '' : 'title="<$Text.ViewEventRegistrationDetailsTooltip$>" ') +
  ...
  {
    WA.Ajax({
      url: '/Sys/Finances/MemberProfileAction/ContinuePendingRegistration',
      dataType: 'json',
      ...
      error: function ()
      {
        console.log('Continue registration error');
      }
    });
  }
  ...
  {
    WA.Ajax({
      url: '/Sys/Finances/MemberProfileAction/CancelPendingRegistration',
      dataType: 'json',
      contentType: "application/json; charset=utf-8",
      global: false,
      type: 'POST',
      ...
      error: function ()
      {
        console.log('Cancel registration error');
      }
    });
  }
  ...
  {
    WA.Ajax({
      url: '/Sys/Finances/MemberProfileAction/CompleteRegistration',
      dataType: 'json',
      contentType: "application/json; charset=utf-8",
      global: false,
      type: 'POST',
      ...
      error: function ()
      {
        console.log('Complete registration error');
      }
    });
  }
  ...
  function fetchRegistrations(filter, searchQuery, page)
  {
    WA.Ajax({
      ...
      type: 'POST',
      url: '/Sys/Finances/MemberProfileAction/GetEventRegistrations',
      dataType: 'json',
      cache: false,
      beforeSend: function() {
        toggleDiv('loadingIndicator', true);
      },
      complete: function() {
        toggleDiv('loadingIndicator', false);
      },
    ...
    <div class="genericListHeaderContainer">
      <table>
        <tbody>
          <tr>
            <td class="filter">
              <div class="eventHeaderLabel">
                <span><$Text.FilterByLabel$></span>
              </div>
            ...
            <td class="searchBox eventSearch">
              ...
              <div>
                <input name="SearchBox" type="text" id="searchBox" class="typeTextBox searchBox" onkeyup="renderTable()">
              </div>
            </td>
    ...
    <div id="idListItemsContainer">
      ...
      <div style="display: none;" id="itemsList">
        <table class="genericListTable" id="membersTable" cellpadding="0" cellspacing="0" width="100%">
          <thead>
            <th class="firstTh">
              <$Text.EventColumnHeader$>
            </th>
            <th>
              <$Text.RegistrationTypeColumnHeader$>
            </th>
            <th class="lastTh rightAlign status">
              <span><$Text.StatusColumnHeader$></span>
            </th>
          </thead>
